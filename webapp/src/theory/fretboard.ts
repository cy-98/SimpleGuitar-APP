...
import { notePitchClass } from "../audio/pitch";
...
/** Pitch class 0–11 at string/fret. */
export function pitchClassAt(string: StringIndex, fret: number): number {
  return ((noteMidiAt(string, fret) % 12) + 12) % 12;
}

/** Build pc → scale degree map for the key (enharmonic spelling from scale). */
...
/** All scale-tone dots on frets 0–12 for the key. */
